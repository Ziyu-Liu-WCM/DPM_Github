...
nBask  <- 400                 # now 400 baskets per scenario
...
for(s in seq_len(nScen)) {
  ...
  # Expand it to 400 x 2:
  #   first 100 rows use row 1's rates,
  #   next 100 rows use row 2's rates,
  #   ...
  #   last 100 rows use row 4's rates
  scenarioDef400 <- do.call(rbind, lapply(seq_len(4), function(i) {
    matrix(scenarioDef[i, ], nrow=100, ncol=2, byrow=TRUE)
  }))
  ...
  for(r in seq_len(nSim)) {
    ...
    # This replicate's data frame: 400 rows (baskets), columns for outcome and rates
    dfRep <- data.frame(
      y1     = numeric(nBask),
      y0     = numeric(nBask),
      n1     = rep(n1, nBask),
    ...
    # Fill in each of the 400 baskets
    for(b in seq_len(nBask)) {
      pi_t <- scenarioDef400[b, 1]  # treatment rate
      pi_c <- scenarioDef400[b, 2]  # control rate
